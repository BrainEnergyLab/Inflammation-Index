...
# pipeline
getAnimalAndTreatmentIDsLegacy <- function(imageStorageDirectory) {
...
read_in_fileLegacy = function(locations, seperators, fileEncoding) {
...
get_file_locationsLegacy = function(morphologyWD, useFrac) {
...
filter_for_vectorLegacy = function(filter_it, filter_by) {
  clean_vec = list()
...
read_in_raw_dataLegacy = function(storageList, TCSExclude) {
  ...
  comboList = lapply(storageList, function(storageList, TCSExclude) {
    ...
    storageList$Files = rbindlist(lapply(storageList$Locations, function(locations, seperators, fileEncoding) {
      ...
      temp = read_in_fileLegacy(locations, seperators, fileEncoding)
      return(temp)
...
fill_to_addLegacy = function(storageListElement) {
...
format_fraclac_filesLegacy = function(fracLacComboList) {
...
remove_unused_metricsLegacy = function(storageList, useFrac) {
...
add_missing_infoLegacy = function(storageList, useFrac) {
  ...
  if(useFrac == T) {
    ...
    storageList$FracLac$Files =  format_fraclac_filesLegacy(storageList$FracLac$Files)
    ...
    for(curr_element in names(toAdd)) {
      toAdd[[curr_element]] = fill_to_addLegacy(storageList[[curr_element]])
    }
...
format_unique_idLegacy = function(storageList, treatmentIDs, animalIDs) {
...
retain_common_cellsLegacy = function(with_id) {
...
convert_hc_to_umLegacy = function(storageList, pixelSize) {
...
format_namesLegacy = function(um_data) {
...
merge_data_togeherLegacy = function(storageList, useFrac) {
...
morphPreProcessingLegacy <- function(pixelSize,
  ...
  passList = get_file_locationsLegacy(morphologyWD, useFrac)
  ...
  for(currType in c('Cell Parameters', 'Sholl Parameters')){
    passList[[currType]]$Locations = filter_for_vectorLegacy(passList[[currType]]$Locations, paste(animalIDs, collapse = '|'))
    passList[[currType]]$Locations = filter_for_vectorLegacy(passList[[currType]]$Locations, paste(treatmentIDs, collapse = '|'))
  }
  
  # Read in our raw csv files
  comboList = read_in_raw_dataLegacy(passList, TCSExclude)
  ...
  mapList = add_missing_infoLegacy(comboList, useFrac)
  
  # Remove columns we no longer need
  cleanList = remove_unused_metricsLegacy(mapList, useFrac)
  ...
  with_id = format_unique_idLegacy(copy(cleanList), treatmentIDs, animalIDs)
  
  # Retain only data where we have cells for all data types
  common_data = retain_common_cellsLegacy(with_id)
  
  if(useFrac == T) {
    
    # Convert our HC data to um
    common_data = convert_hc_to_umLegacy(common_data, pixelSize)
    
  }
  
  # Format all our metric names so we can merge data types
  name_changed = format_namesLegacy(common_data)
  
  # Merge our data types into a single data.table
  merged = merge_data_togeherLegacy(name_changed, useFrac)
